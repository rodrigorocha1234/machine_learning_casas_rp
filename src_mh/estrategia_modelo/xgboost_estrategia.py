...
class XGBoostEstrategia(
    EstrategiaModelo[pd.DataFrame, pd.Series, np.ndarray]
):
    """Estratégia Concreta de Machine Learning utilizando Extreme Gradient Boosting (XGBoost Regressor)."""

    def __init__(self, params: dict | None = None):
        self.__params = params or {
            "n_estimators": getattr(Config, "n_estimators_xgb", 150),
            "learning_rate": getattr(Config, "learning_rate_xgb", 0.08),
            "max_depth": getattr(Config, "max_depth_xgb", 5),
            "subsample": getattr(Config, "subsample_xgb", 0.85),
            "colsample_bytree": getattr(Config, "colsample_bytree_xgb", 0.85),
            "reg_alpha": getattr(Config, "reg_alpha_xgb", 0.1),
            "reg_lambda": getattr(Config, "reg_lambda_xgb", 1.0),
            "random_state": 42,
            "n_jobs": 4,
        }
        self.__modelo = XGBRegressor(**self.__params)
        self.__colunas: list[str] = []
        self.__equacoes_por_zona: dict[str, float] = {}
        self.__params_tuning = {
            "n_estimators": getattr(Config, "n_estimators_xgb_turing", [50, 100, 150, 200]),
            "learning_rate": getattr(Config, "learning_rate_xgb_turing", [0.03, 0.08, 0.15]),
            "max_depth": getattr(Config, "max_depth_xgb_turing", [3, 5, 7]),
            "subsample": getattr(Config, "subsample_xgb_turing", [0.7, 0.85, 1.0]),
        }
    ...
    def treinar(self, x_train: pd.DataFrame, y_train: pd.Series) -> None:
        ...
        # Calcula o preço médio estimado pelo XGBoost por Zona do Bairro
    ...
    def obter_equacao_reta_geral(self) -> str:
        ...
        resumo_str = "\n".join(resumo_zonas)
        return (
            f"Extreme Gradient Boosting Regressor (XGBoost — {n_arvores} Árvores com Regularização de 2ª Ordem)\n"
            f"• Taxa de Aprendizado (Eta): {lr} | Profundidade Máxima: {depth}\n"
            f"• Regularização L1 (Alpha): {l1} | Regularização L2 (Lambda): {l2}\n\n"
            f"{resumo_str}"
        )
    ...
    def obter_curva_validacao(
        ...
        """Calcula as pontuações da curva de validação variando n_estimators no XGBoost."""
        train_scores, test_scores = validation_curve(
            XGBRegressor(
                learning_rate=self.__params.get("learning_rate", 0.08),
                max_depth=self.__params.get("max_depth", 5),
    ...
    def _plotar_diagnostico_overfitting_underfitting(
        ...
        """Gera a figura Matplotlib com as particularidades do XGBoost (n_estimators)."""
        if not dados:
            return None
        ...
        ax.plot(x, y_tr, "-o", color="#1f77b4", linewidth=2.5, markersize=7, label="RMSE Treino (Gradiente 2ª Ordem)", zorder=4)
        ...
        if len(x) > 1:
            ...
            if min_val_idx > 0:
                ax.axvspan(x[0], x_opt_start, color="#ffebee", alpha=0.4, label="Região de Subajuste de Etapas XGBoost", zorder=1)
            ax.axvspan(x_opt_start, x_opt_end, color="#e8f5e9", alpha=0.5, label="Região de Ajuste Ótimo XGBoost", zorder=1)
        ...
        if min_val_idx > 0:
            ax.text(x[0], y_val[0] + y_range * 0.08, " [ POUCAS ÁRVORES ] \n (Subestimativa de Gradiente / Underfitting) ", fontsize=9, fontweight="bold", color="#c62828", va="bottom", ha="left", bbox=dict(boxstyle="round,pad=0.4", facecolor="#ffffff", edgecolor="#e57373", alpha=0.95), zorder=5)

        ax.annotate(
            f" [ AJUSTE ÓTIMO ] (Extreme Gradient Boosting)\n Melhor n_estimators = {best_param_str}\n RMSE Validação ≈ R$ {min_val_rmse:,.0f} ".replace(",", "."),
        ...
        if len(x) > 1 and min_val_idx < len(x) - 1:
            ax.text(x[-1], (y_tr[-1] + y_val[-1]) / 2.0, " [ ESTABILIZAÇÃO ] \n (Convergência de 2ª Ordem L1/L2) ", fontsize=9, fontweight="bold", color="#1a237e", va="center", ha="right", bbox=dict(boxstyle="round,pad=0.4", facecolor="#ffffff", edgecolor="#7986cb", alpha=0.95), zorder=5)

        ax.set_title(f"{self.nome} — Diagnóstico Preditivo (Quantidade de Árvores - n_estimators)", fontsize=13.5, fontweight="bold", pad=15)
        ax.set_xlabel(f"{param_name} (Número de Árvores de Decisão no XGBoost)", fontsize=10.5, fontweight="bold", labelpad=10)
    ...
    def realizar_grid_search(
        ...
        """Executa busca em grade de hiperparâmetros (GridSearchCV) para XGBoost."""
    ...
    def realizar_validacao_cruzada(
        ...
        """Executa 1 iteração de K-Fold Cross-Validation para o XGBoost."""
    ...
    def plotar_importancia_atributos(self) -> plt.Figure | None:
        """Gera um gráfico de barras horizontais rico e legível com a importância dos atributos (Gain Importance)."""
        if not hasattr(self.__modelo, "feature_importances_") or len(self.__colunas) == 0:
            return None
        ...
        ax.set_title(
            f"{self.nome} — Importância Relativa dos Atributos (Gain / Importance)",
            fontsize=13.5,
            fontweight="bold",
            pad=15,
        )
        ax.set_xlabel("Importância Relativa (%) — Ganho Médio de Divisão de Árvore (Gain)", fontsize=10.5, fontweight="bold", labelpad=10)
    ...
    def obter_resultados(
        ...
        max_depth = int(getattr(self.__modelo, "max_depth", 5))
