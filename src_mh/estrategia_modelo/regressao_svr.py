...
class RegressaoSVREstrategia(
    EstrategiaModelo[pd.DataFrame, pd.Series, np.ndarray]
):
    """Estratégia Concreta de Machine Learning utilizando Regressão por Vetores de Suporte (Support Vector Regression - SVR)."""

    def __init__(self, params: dict | None = None):
        self.__params = params or {
            "C": getattr(Config, "c_svr", 100.0),
            "epsilon": getattr(Config, "epsilon_svr", 0.1),
            "kernel": getattr(Config, "kernel_svr", "rbf"),
            "gamma": getattr(Config, "gamma_svr", "scale"),
        }

        inner_pipeline = Pipeline([
            ("scaler", StandardScaler()),
            ("svr", SVR(
                C=self.__params.get("C", 100.0),
                epsilon=self.__params.get("epsilon", 0.1),
                kernel=self.__params.get("kernel", "rbf"),
                gamma=self.__params.get("gamma", "scale"),
            )),
        ])
        ...
        self.__params_tuning = {
            "regressor__svr__C": getattr(Config, "c_svr_turing", [1.0, 10.0, 100.0, 1000.0]),
            "regressor__svr__epsilon": getattr(Config, "epsilon_svr_turing", [0.01, 0.1, 0.2]),
            "regressor__svr__kernel": getattr(Config, "kernel_svr_turing", ["rbf", "linear"]),
        }
        self.__param_range = np.logspace(-1, 3, 10)  # C variando de 0.1 a 1000.0

    @property
    @override
    def nome(self) -> str:
        return "Regressão SVR"
    ...
    def treinar(self, x_train: pd.DataFrame, y_train: pd.Series) -> None:
        ...
        # Calcula o preço médio estimado pelo SVR por Zona do Bairro
    ...
    def obter_equacoes_por_zona(self) -> dict[str, float]:
        """Preço médio estimado pelo SVR para cada Zona do Bairro."""
        return self.__equacoes_por_zona

    @override
    def obter_equacao_reta_geral(self) -> str:
        """Exporta a descrição matemática do modelo SVR (Kernel, Hiperparâmetros e Zonas do Bairro)."""
        svr_obj = self.__obter_svr_estimator()
        c_val = getattr(svr_obj, "C", self.__params.get("C", 100.0))
        eps_val = getattr(svr_obj, "epsilon", self.__params.get("epsilon", 0.1))
        kernel_val = getattr(svr_obj, "kernel", self.__params.get("kernel", "rbf"))
        n_support = (
            int(np.sum(svr_obj.n_support_))
            if hasattr(svr_obj, "n_support_")
            else 0
        )

        resumo_zonas = ["=== PREÇO MÉDIO PREVISTO POR ZONA DO BAIRRO (SVR) ==="]
        ...
        resumo_str = "\n".join(resumo_zonas)
        return (
            f"Support Vector Regression (SVR — Kernel: {kernel_val.upper()}, C: {c_val}, Epsilon: {eps_val}, "
            f"Vetores de Suporte: {n_support})\n\n"
            f"{resumo_str}"
        )

    def __obter_svr_estimator(self) -> SVR:
        """Recupera a instância do estimador SVR dentro do TransformedTargetRegressor e Pipeline."""
        if hasattr(self.__modelo, "regressor_") and hasattr(self.__modelo.regressor_, "named_steps"):
            return self.__modelo.regressor_.named_steps.get("svr", self.__modelo.regressor_)
        elif hasattr(self.__modelo, "regressor") and hasattr(self.__modelo.regressor, "named_steps"):
            return self.__modelo.regressor.named_steps.get("svr", self.__modelo.regressor)
        return SVR()
    ...
    def obter_curva_validacao(
        ...
        """Calcula as pontuações da curva de validação variando o hiperparâmetro C do SVR."""
    ...
    def _plotar_diagnostico_overfitting_underfitting(
        ...
        """Gera a figura Matplotlib com as particularidades do SVR (Hiperparâmetro C de Regularização)."""
        if not dados:
            return None

        param_name = dados.get("param_name", "C")
        param_range = dados.get("C_range") or dados.get("param_range", [])
        ...
        ax.plot(x, y_tr, "-o", color="#1f77b4", linewidth=2.5, markersize=7, label="RMSE Treino (Viés SVR)", zorder=4)
        ...
        best_param_str = f"{best_param:.2f}" if best_param < 10 else f"{int(best_param)}"
        ...
        if len(x) > 1:
            ...
            if min_val_idx > 0:
                ax.axvspan(x[0], x_opt_start, color="#ffebee", alpha=0.4, label="Região de Underfitting (C Baixo / Margem Larga)", zorder=1)
            ax.axvspan(x_opt_start, x_opt_end, color="#e8f5e9", alpha=0.5, label="Região de Ajuste Ótimo SVR", zorder=1)
            if min_val_idx < len(x) - 1:
                ax.axvspan(x_opt_end, x[-1], color="#fff8e1", alpha=0.4, label="Região de Overfitting (C Alto / Penalização Rígida)", zorder=1)

        ax.axvline(best_param, color="#2e7d32", linestyle="--", linewidth=2.2, label=f"Melhor C = {best_param_str}", zorder=4)
        ax.plot(best_param, min_val_rmse, "o", color="#ff7f0e", markeredgecolor="#2e7d32", markeredgewidth=2.5, markersize=12, zorder=6)

        ax.text(x[0], y_val[0] + y_range * 0.08, " [ UNDERFITTING ] \n (C Baixo / Margem Excessivamente Tolerante) ", fontsize=9, fontweight="bold", color="#c62828", va="bottom", ha="left", bbox=dict(boxstyle="round,pad=0.4", facecolor="#ffffff", edgecolor="#e57373", alpha=0.95), zorder=5)

        ax.annotate(
            f" [ AJUSTE ÓTIMO ] (Equilíbrio SVR)\n Melhor C = {best_param_str}\n RMSE Validação ≈ R$ {min_val_rmse:,.0f} ".replace(",", "."),
            xy=(best_param, min_val_rmse),
            xytext=(best_param * 0.05 if best_param > 1 else best_param * 0.3, min_val_rmse + y_range * 0.12),
            fontsize=9,
            fontweight="bold",
            color="#1b5e20",
            ha="center",
        ...
        if len(x) > 1 and min_val_idx < len(x) - 1:
            ax.text(x[-1], (y_tr[-1] + y_val[-1]) / 2.0, " [ OVERFITTING ] \n (C Elevado / Ajuste Rígido aos Ruídos) ", fontsize=9, fontweight="bold", color="#f57f17", va="center", ha="right", bbox=dict(boxstyle="round,pad=0.4", facecolor="#ffffff", edgecolor="#fbc02d", alpha=0.95), zorder=5)

        ax.set_title(f"{self.nome} — Diagnóstico Preditivo (Parâmetro de Regularização C)", fontsize=13.5, fontweight="bold", pad=15)
        ax.set_xlabel(f"{param_name} (Parâmetro de Penalização C — Escala Logarítmica)", fontsize=10.5, fontweight="bold", labelpad=10)
    ...
    def realizar_grid_search(
        ...
        """Executa busca em grade de hiperparâmetros (GridSearchCV) para SVR."""
    ...
    def realizar_validacao_cruzada(
        ...
        """Executa 1 iteração de K-Fold Cross-Validation para o SVR."""
    ...
    def obter_resultados(
        ...
        eps_val = getattr(svr_obj, "epsilon", self.__params.get("epsilon", 0.1))
        kernel_val = getattr(svr_obj, "kernel", self.__params.get("kernel", "rbf"))
        gamma_val = getattr(svr_obj, "gamma", self.__params.get("gamma", "scale"))
        n_support = (
            int(np.sum(svr_obj.n_support_))
            if hasattr(svr_obj, "n_support_")
            else 0
        ...
        return {
            "C_range": self.__param_range.tolist(),
            ...
            "kernel": str(kernel_val),
            "gamma": str(gamma_val),
            "numero_vetores_suporte": n_support,
