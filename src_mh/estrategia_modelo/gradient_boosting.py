...
class GradientBoostingEstrategia(
    EstrategiaModelo[pd.DataFrame, pd.Series, np.ndarray]
):
    """Estratégia Concreta de Machine Learning utilizando Gradient Boosting Regressor (Ensemble Sequencial por Boosting)."""

    def __init__(self, params: dict | None = None):
        self.__params = params or {
            "n_estimators": getattr(Config, "n_estimators_gb", 150),
            "learning_rate": getattr(Config, "learning_rate_gb", 0.1),
            "max_depth": getattr(Config, "max_depth_gb", 4),
            "min_samples_split": getattr(Config, "min_samples_split_gb", 5),
            "min_samples_leaf": getattr(Config, "min_samples_leaf_gb", 2),
            "subsample": getattr(Config, "subsample_gb", 0.9),
            "random_state": 42,
        }
        self.__modelo = GradientBoostingRegressor(**self.__params)
        self.__colunas: list[str] = []
        self.__equacoes_por_zona: dict[str, float] = {}
        self.__params_tuning = {
            "n_estimators": getattr(Config, "n_estimators_gb_turing", [50, 100, 150, 200]),
            "learning_rate": getattr(Config, "learning_rate_gb_turing", [0.05, 0.1, 0.2]),
            "max_depth": getattr(Config, "max_depth_gb_turing", [3, 4, 5]),
        }
    ...
    def treinar(self, x_train: pd.DataFrame, y_train: pd.Series) -> None:
        ...
        # Calcula o preço médio estimado pelo Gradient Boosting por Zona do Bairro
    ...
    def obter_equacoes_por_zona(self) -> dict[str, float]:
        """Preço médio estimado pelo Gradient Boosting para cada Zona do Bairro."""
        return self.__equacoes_por_zona

    @override
    def obter_equacao_reta_geral(self) -> str:
        """Exporta a descrição do modelo Gradient Boosting e as estimativas médias por Zona do Bairro."""
        if not hasattr(self.__modelo, "estimators_") or len(self.__colunas) == 0:
            return "Gradient Boosting não treinado."

        n_arvores = len(self.__modelo.estimators_)
        lr = self.__params.get("learning_rate", 0.1)
        depth = self.__params.get("max_depth", 4)

        resumo_zonas = ["=== PREÇO MÉDIO PREVISTO POR ZONA DO BAIRRO (GRADIENT BOOSTING) ==="]
        ...
        resumo_str = "\n".join(resumo_zonas)
        return (
            f"Gradient Boosting Regressor (Ensemble Sequencial: {n_arvores} Árvores de Resíduo, "
            f"Taxa de Aprendizado: {lr}, Profundidade Máx: {depth})\n\n"
            f"{resumo_str}"
        )
    ...
    def obter_curva_validacao(
        ...
        """Calcula as pontuações da curva de validação variando n_estimators no Gradient Boosting."""
        train_scores, test_scores = validation_curve(
            GradientBoostingRegressor(
                learning_rate=self.__params.get("learning_rate", 0.1),
                max_depth=self.__params.get("max_depth", 4),
                min_samples_split=self.__params.get("min_samples_split", 5),
                min_samples_leaf=self.__params.get("min_samples_leaf", 2),
                subsample=self.__params.get("subsample", 0.9),
                random_state=42,
            ),
            x,
            y,
    ...
    def _plotar_diagnostico_overfitting_underfitting(
        ...
        """Gera a figura Matplotlib com as particularidades do Gradient Boosting (n_estimators)."""
        if not dados:
            return None
        ...
        ax.plot(x, y_tr, "-o", color="#1f77b4", linewidth=2.5, markersize=7, label="RMSE Treino (Gradiente Residual)", zorder=4)
        ...
        if len(x) > 1:
            ...
            if min_val_idx > 0:
                ax.axvspan(x[0], x_opt_start, color="#ffebee", alpha=0.4, label="Região de Subajuste de Etapas Boosting", zorder=1)
            ax.axvspan(x_opt_start, x_opt_end, color="#e8f5e9", alpha=0.5, label="Região de Ajuste Ótimo de Boosting", zorder=1)
            if min_val_idx < len(x) - 1:
                ax.axvspan(x_opt_end, x[-1], color="#e8eaf6", alpha=0.4, label="Região de Estabilização Residual", zorder=1)
        ...
        if min_val_idx > 0:
            ax.text(x[0], y_val[0] + y_range * 0.08, " [ POUCAS ETAPAS ] \n (Subestimativa Residual / Underfitting) ", fontsize=9, fontweight="bold", color="#c62828", va="bottom", ha="left", bbox=dict(boxstyle="round,pad=0.4", facecolor="#ffffff", edgecolor="#e57373", alpha=0.95), zorder=5)

        ax.annotate(
            f" [ AJUSTE ÓTIMO ] (Gradient Boosting)\n Melhor n_estimators = {best_param_str}\n RMSE Validação ≈ R$ {min_val_rmse:,.0f} ".replace(",", "."),
        ...
        if len(x) > 1 and min_val_idx < len(x) - 1:
            ax.text(x[-1], (y_tr[-1] + y_val[-1]) / 2.0, " [ ESTABILIZAÇÃO ] \n (Convergência de Resíduos) ", fontsize=9, fontweight="bold", color="#1a237e", va="center", ha="right", bbox=dict(boxstyle="round,pad=0.4", facecolor="#ffffff", edgecolor="#7986cb", alpha=0.95), zorder=5)

        ax.set_title(f"{self.nome} — Diagnóstico Preditivo (Quantidade de Etapas Boosting - n_estimators)", fontsize=13.5, fontweight="bold", pad=15)
        ax.set_xlabel(f"{param_name} (Número de Árvores Sequenciais no Boosting)", fontsize=10.5, fontweight="bold", labelpad=10)
    ...
    def realizar_grid_search(
        ...
        """Executa busca em grade de hiperparâmetros (GridSearchCV) para Gradient Boosting."""
    ...
    def realizar_validacao_cruzada(
        ...
        """Executa 1 iteração de K-Fold Cross-Validation para o Gradient Boosting."""
    ...
    def plotar_importancia_atributos(self) -> plt.Figure | None:
        ...
        ax.set_xlabel("Importância Relativa (%) — Ganho Médio de Decréscimo de Impureza (MDI)", fontsize=10.5, fontweight="bold", labelpad=10)
    ...
    def obter_resultados(
        ...
        learning_rate = float(getattr(self.__modelo, "learning_rate", 0.1))
        max_depth = int(getattr(self.__modelo, "max_depth", 4))
