...
class RegressaoRidgeEstrategia(
    ...
    def _plotar_diagnostico_overfitting_underfitting(
        ...
        """Gera a figura Matplotlib com as particularidades da Regressão Ridge (Penalização L2 / Suavização)."""
        if not dados:
            return None
        ...
        ax.plot(x, y_tr, "-o", color="#1f77b4", linewidth=2.5, markersize=7, label="RMSE Treino (Viés L2)", zorder=4)
        ...
        if len(x) > 1:
            ...
            if min_val_idx > 0:
                ax.axvspan(x[0], x_opt_start, color="#fff8e1", alpha=0.4, label="Região de Overfitting (Sem Penalização L2)", zorder=1)
            ax.axvspan(x_opt_start, x_opt_end, color="#e8f5e9", alpha=0.5, label="Região de Ajuste Ótimo Ridge", zorder=1)
        ...
        ax.axvline(best_param, color="#2e7d32", linestyle="--", linewidth=2.2, label=f"Melhor alpha L2 = {best_param_str}", zorder=4)
        ax.plot(best_param, min_val_rmse, "o", color="#ff7f0e", markeredgecolor="#2e7d32", markeredgewidth=2.5, markersize=12, zorder=6)

        ax.text(x[0], y_tr[0] + y_range * 0.15, " [ OVERFITTING ] \n (Sem Penalização L2 / Coeficientes Livres) ", fontsize=9, fontweight="bold", color="#f57f17", va="bottom", ha="left", bbox=dict(boxstyle="round,pad=0.4", facecolor="#ffffff", edgecolor="#fbc02d", alpha=0.95), zorder=5)
        ax.annotate(
            f" [ AJUSTE ÓTIMO ] (Penalização L2 - Suavização)\n Melhor alpha = {best_param_str}\n RMSE Validação ≈ R$ {min_val_rmse:,.0f} ".replace(",", "."),
        ...
        ax.text(x[-1], y_tr[-1] + y_range * 0.08, " [ UNDERFITTING ] \n (Penalização L2 Excessiva / Coeficientes Suavizados) ", fontsize=9, fontweight="bold", color="#c62828", va="bottom", ha="right", bbox=dict(boxstyle="round,pad=0.4", facecolor="#ffffff", edgecolor="#e57373", alpha=0.95), zorder=5)

        ax.set_title(f"{self.nome} — Diagnóstico Preditivo (Penalização L2 - Ridge)", fontsize=13.5, fontweight="bold", pad=15)
        ax.set_xlabel(f"{param_name} (Fator de Penalização L2 — Escala Logarítmica)", fontsize=10.5, fontweight="bold", labelpad=10)
