...
class RegressaoLassoEstrategia(
    ...
    def __init__(self, params: dict | None = None):
        self.__params = params or {
            ...
            "max_iter": getattr(Config, "max_iter_lasso", 2000),
        }
    ...
    def obter_curva_validacao(
        ...
        train_scores, test_scores = validation_curve(
            Lasso(fit_intercept=self.__modelo.fit_intercept, max_iter=2000),
            x,
            y,
            param_name="alpha",
    ...
    def gerar_figura_underfit_overfit(self, dados: dict[str, Any]) -> plt.Figure | None:
        """Gera a figura Matplotlib destacando visualmente Bias vs Variance para Regressão Lasso."""
        if not dados:
        ...
        ax.set_xlabel("alpha (Regularização L1)")
        ax.set_ylabel("RMSE")
        ax.set_title("Regressão Lasso — Bias vs Variance")
        ax.legend()
        ax.grid(True)
        fig.tight_layout()
        ...
        try:
            if mlflow.active_run():
                ...
                buf.seek(0)
                img = Image.open(buf)
                mlflow.log_image(img, "under_over_lasso.png")
